...
import { SendForgotPasswordOTP, ResetPassword } from '../../API/LoginAPI/ForgotPassword'; // Import forgot password APIs
...
const DonorLogin = () => {
  ...
  const handleLogin = async ({ email, password }) => {
    ...
    try {
      ...
      if (data?.token) {
          if (data.role !== 'DONOR') {
          setMessage({ text: 'Access denied. Only donor can login here.'});
          return;
        }
        ...
        setTimeout(() => navigate(pendingSubmission ? "/donor/dashboard" : "/donor/dashboard"), 1000);
      } else {
        setMessage({
        ...
        <div className="relative z-10">
          ...
          <p className="text-center text-gray-500 mb-8">
            {showForgotPassword 
              ...
              : (!isLogin ? "Create a donor account to start helping" : "Login to your donor portal")
            }
          </p>

          {message.text && (
            <div className={`mb-6 p-4 rounded-xl text-sm text-center font-medium animate-in fade-in slide-in-from-top-2 duration-300 ${
